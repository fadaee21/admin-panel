...
const EditCoupon = () => {
  ...
  const {
    ...
    router.query.id && `/global?url=/coupons/${router.query.id}`
  );
  ...
  const onSubmit = async (data: Coupon) => {
    try {
      setLoading(true);
      const res = await apiAxiosApp.put(
        `/global?url=/coupons/${router.query.id}`,
        {
          ...data,
          expired_at:
            dateExpire instanceof DateObject
              ? dateExpire
  ...
  return (
    <>
      <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h4 className="fw-bold">ویرایش تخفیف : {coupon.code} </h4>
      </div>
      <form onSubmit={handleSubmit(onSubmit)} className="row">
        <div className="col-md-3">
          <label className="form-label">کد</label>
          <input
            {...register("code", { required: "فیلد کد الزامی است" })}
            type="text"
        ...
        <div className="col-md-3">
          ...
          <input
            {...register("percentage", { required: "فیلد درصد الزامی است" })}
            type="text"
